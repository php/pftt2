...
public abstract class CharsetRecog_mbcs extends CharsetRecognizer {
   ...
   /**
    ...
    int match(CharsetDetector det, int [] commonChars) {
    	return match(det, commonChars, new iteratedChar());
    }
    @Override
    public void match(final byte[] input, final int len, CharsetRec cr) {
    	int[] commonChars = this.getCommonChars();
        ...
        iteratedChar iter = new iteratedChar() {
        	public int nextByte(CharsetDetector det) {
                ...
                int byteValue = (int)input[nextIndex++] & 0x00ff;
                return byteValue;
        	}
        };
        
        detectBlock: {
            for (iter.reset(); nextChar(iter, null);) {
                totalCharCount++;
                if (iter.error) {
                    badCharCount++; 
                } else {
    ...
    public int match(CharsetDetector det, int [] commonChars, iteratedChar iter) {
        @SuppressWarnings("unused")
        int   singleByteCharCount = 0;  //TODO Do we really need this?
        int   doubleByteCharCount = 0;
        int   commonCharCount     = 0;
     ...
     public static class iteratedChar {
         public int             charValue = 0;             // 1-4 bytes from the raw input data
         public int             index     = 0;
      ...
      */
     public abstract boolean nextChar(iteratedChar it, CharsetDetector det);
     ...
     public static class CharsetRecog_sjis extends CharsetRecog_mbcs {
         ...
         @Override
         public int[] getCommonChars() {
        	 return commonChars;
         }
         @Override
 		public ERecognizedLanguage getLang() {
 			return ERecognizedLanguage.JP;
 		}
      ...
      *
      */
     public abstract static class CharsetRecog_euc extends CharsetRecog_mbcs {
         ...
         public static class CharsetRecog_euc_kr extends CharsetRecog_euc {
             ...
             @Override
     		public ERecognizedLanguage getLang() {
     			return ERecognizedLanguage.KR;
     		}
             @Override
             protected Charset getCharset() {
            	 return CharsetRec.EUC_KR;
             }
