...
class CharsetLMBCS extends CharsetICU {
    ...
    class CharsetDecoderLMBCS extends CharsetDecoderICU {
        ...
        private int LMBCS_SimpleGetNextUChar(UConverterSharedData cnv, ByteBuffer source, int positionOffset, int length, CharsetDecoderListener cdl) {
            int uniChar;
            int oldSourceLimit;
            int oldSourcePos;
            ...
            uniChar = extraInfo.decoder.simpleGetNextUChar(source, false, cdl);
         ...
         */
        private int LMBCSGetNextUCharWorker(ByteBuffer source, CoderResult[] err, CharsetDecoderListener cdl) {
                ...
                if (CurByte == ULMBCS_GRP_CTRL) {  /* Control character group - no opt group update */
                    ...
                    if (group > ULMBCS_GRP_LAST || (cnv = extraInfo.OptGrpConverter[group]) == null) {
                        ...
                        if (source.get(source.position()) == group) {
                            /* single byte */
                            source.get();
                            uniChar = LMBCS_SimpleGetNextUChar(cnv, source, 0, 1, cdl);
                            source.get();
                        } else {
                            /* double byte */
                            uniChar = LMBCS_SimpleGetNextUChar(cnv, source, 0, 2, cdl);
                            source.get();
                            source.get();
                        }
                        ...
                        if (CurByte >= ULMBCS_C1START) {
                            ...
                            uniChar = LMBCS_SimpleGetNextUChar(cnv, ByteBuffer.wrap(bytes), 0, 2, cdl);
                        }
                    }
                    ...
                    if (group >= ULMBCS_DOUBLEOPTGROUP_START) { /* double byte conversion */
                        if (CharsetMBCS.MBCS_ENTRY_IS_TRANSITION(cnv.mbcs.stateTable[0][CurByte]) /* isLeadByte */) {
                            ...
                            uniChar = LMBCS_SimpleGetNextUChar(cnv, source, -1, 1, cdl);
                        } else {
                            /* CHECK_SOURCE_LIMIT(1) */
                            if (source.position() + 1 > source.limit()) {
                                err[0] = CoderResult.OVERFLOW;
                            ...
                            /* let the MBCS conversion consume CurByte again */
                            uniChar = LMBCS_SimpleGetNextUChar(cnv, source, -1, 2, cdl);
                            source.get();
                        }
                    } else {
                        uniChar = CharsetMBCS.MBCS_SINGLE_SIMPLE_GET_NEXT_BMP(cnv.mbcs, CurByte);
        ...
        protected CoderResult decodeLoop(ByteBuffer source, CharBuffer target, IntBuffer offsets, boolean flush, CharsetDecoderListener cdl) { 
            ...
            while (err[0].isUnderflow() && source.hasRemaining() && target.hasRemaining()) {
                ...
                if (toULength > 0) { /* reassemble char from previous call */
                    ...
                    uniChar = (char)LMBCSGetNextUCharWorker(tmpSourceBuffer, err, cdl);
                    ...
                    uniChar = (char)LMBCSGetNextUCharWorker(source, err, cdl);
