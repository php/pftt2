...
class CharsetSCSU extends CharsetICU{
  ...
  //  };
    ...
    class CharsetDecoderSCSU extends CharsetDecoderICU {       
                ...
                if(isSingleByteMode){
                    switch(labelType){
                        case FastSingle:
                            ...
                            labelType = fastSingle(source, target, offsets, ByteMode, cdl);
                            break;
                        case SingleByteMode:
                            /* normal state machine for single-byte mode, minus handling for what fastSingleCovers */
                            labelType = singleByteMode(source, target, offsets, ByteMode, cdl);
                            break;
                        case EndLoop:
                    ...
                    switch(labelType){
                        case FastSingle:
                            /*fast path for single-byte mode*/
                            labelType = fastSingle(source, target, offsets, UnicodeMode, cdl);
                            break;
                        case SingleByteMode:
                            /* normal state machine for single-byte mode, minus handling for what fastSingleCovers */
                            labelType = singleByteMode(source, target, offsets, UnicodeMode, cdl);
                            break;
                        case EndLoop:
                            endLoop(source, target, offsets);
                            break;
        ...
        private int fastSingle(ByteBuffer source, CharBuffer target, IntBuffer offsets, int modeType, CharsetDecoderListener cdl){
            int label = 0;
            if(modeType==ByteMode){
                ...
                if(state==readCommand){
                    while(true) {
                    	...
                    	if (!(source.hasRemaining() && target.hasRemaining() && (b=(short)(byte_b & UConverterConstants.UNSIGNED_BYTE_MASK)) >= 0x20)) 
                    		break;
                        ...
                        if(b <= 0x7f){
                            ...
                            if(c <= 0xffff){
                                ...
                                if (cdl!=null)
                                	cdl.decoded(byte_b, (char)(0xd7c0 + (c>>10)));
                                if(target.hasRemaining()){
                                    target.put((char)(0xdc00 | (c&0x3ff)));
                                    if (cdl!=null)
                                    	cdl.decoded(byte_b, (char)(0xdc00 | (c&0x3ff)));
                                    if(offsets != null){
                                        offsets.put(sourceIndex);
                                        offsets.put(sourceIndex);
                                    }
                ...
                if(state == readCommand){
                    while((source.position()+1)<source.limit() && target.hasRemaining() && (((b=source.get(source.position()))-UC0)&UConverterConstants.UNSIGNED_BYTE_MASK)>(Urs-UC0)){
                    	byte b = source.get(source.position()+1);
                        target.put((char)((b<<8)|(b&UConverterConstants.UNSIGNED_BYTE_MASK)));
                        if (cdl!=null)
                        	cdl.decoded(b, (char)((b<<8)|(b&UConverterConstants.UNSIGNED_BYTE_MASK)));
                        if(offsets != null){
                            offsets.put(sourceIndex);
                        }
                        sourceIndex = nextSourceIndex;
        ...
        private int singleByteMode(ByteBuffer source, CharBuffer target, IntBuffer offsets, int modeType, CharsetDecoderListener cdl){
            ...
            if(modeType == ByteMode){
                while(source.hasRemaining()){
                    ...
                    case quotePairTwo:
                        ...
                        if (cdl!=null)
                        	cdl.decoded(byte_b, (char)((byteOne<< 8) | b));
                        if(offsets != null){
                            offsets.put(sourceIndex);
                        }
                        sourceIndex = nextSourceIndex;
                    ...
                    case quoteOne:
                        if(b<0x80){
                            ...
                            if (cdl!=null)
                            	cdl.decoded(byte_b, (char)(staticOffsets[quoteWindow] + b));
                            if(offsets != null){
                                offsets.put(sourceIndex);
                            }
                        }else {
