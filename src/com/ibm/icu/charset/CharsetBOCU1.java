    ...
    static class CharsetDecoderBOCU extends CharsetDecoderICU{
        ...
        protected CoderResult decodeLoop(ByteBuffer source, CharBuffer target, IntBuffer offsets,
            ...
            while(LabelLoop){
                switch(labelType){
                    case fastSingle:
                        labelType = fastSingle(source, target, offsets, cdl);
                        break;
                    case getTrail:
                        labelType = getTrail(source, target, offsets);
                        break;
                    case regularLoop:
                        labelType = afterGetTrail(source, target, offsets, cdl);
                        break;
                    case endLoop:
        ...
        private int fastSingle(ByteBuffer source, CharBuffer target, IntBuffer offsets, CharsetDecoderListener cdl){
            ...
            byte b;
            while(count>0) {
            	b = source.get(source.position());
                if(BOCU1_START_NEG_2 <=(c=b&UConverterConstants.UNSIGNED_BYTE_MASK) && c< BOCU1_START_POS_2) {
                    c = prev + (c-BOCU1_MIDDLE);
                    if(c<0x3000) {
                        target.put((char)c);
                        
                        if (cdl!=null)
                        	cdl.decoded(b, (char)c);
        ...
        private int afterGetTrail(ByteBuffer source, CharBuffer target, IntBuffer offsets, CharsetDecoderListener cdl){
            ...
            while(afterTrail || source.hasRemaining()) {
                if(!afterTrail){
                    ...
                    ++nextSourceIndex;
                    b = source.get();
                    c = b&UConverterConstants.UNSIGNED_BYTE_MASK;
