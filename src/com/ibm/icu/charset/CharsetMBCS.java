...
//        size = mbcsTable.countStates * 1024 + sizeofFromUBytes + UConverterConstants.MAX_CONVERTER_NAME_LENGTH + 20;
    ...
    class CharsetDecoderMBCS extends CharsetDecoderICU {
        ...
        @Override
        protected CoderResult decodeLoop(ByteBuffer source, CharBuffer target, IntBuffer offsets, boolean flush, CharsetDecoderListener cdl) {
        /* Just call cnvMBCSToUnicodeWithOffsets() to remove duplicate code. */
            return cnvMBCSToUnicodeWithOffsets(source, target, offsets, flush, cdl);
        }
        ...
        private CoderResult continueMatchToU(ByteBuffer source, CharBuffer target, IntBuffer offsets, int srcIndex,
            ...
            match = matchToU((byte) SISO_STATE(sharedData, mode), preToUArray, preToUBegin, preToULength, source,
                    value, isToUUseFallback(), flush, cdl);
        ...
        private int matchToU(byte sisoState, byte[] preArray, int preArrayBegin, int preLength, ByteBuffer source,
                int[] pMatchValue, boolean isUseFallback, boolean flush, CharsetDecoderListener cdl) {
            ...
            for (;;) {
                ...
                if (i < preLength) {
                	...
                	byte byte_b = source.get(source.position() + j++);
                	if (cdl!=null)
                		cdl.push(byte_b);
                    b = (short) (byte_b & UConverterConstants.UNSIGNED_BYTE_MASK);
                } else {
        ...
        private int toU(int length, ByteBuffer source, CharBuffer target, IntBuffer offsets, int sourceIndex,
                boolean flush, CoderResult[] cr, CharsetDecoderListener cdl) {
            // ByteBuffer cx;

            if (sharedData.mbcs.extIndexes != null
                    && initialMatchToU(length, source, target, offsets, sourceIndex, flush, cr, cdl)) {
        ...
        private boolean initialMatchToU(int firstLength, ByteBuffer source, CharBuffer target, IntBuffer offsets,
                int srcIndex, boolean flush, CoderResult[] cr, CharsetDecoderListener cdl) {
            int[] value = new int[1];
            int match = 0;
        ...
        private int simpleMatchToU(ByteBuffer source, boolean useFallback, CharsetDecoderListener cdl) {
            int[] value = new int[1];
            int match;
            ...
            match = matchToU((byte) -1, sourceArray, sourcePosition, sourceLimit, null, value, useFallback, true, cdl);
        ...
        CoderResult cnvMBCSToUnicodeWithOffsets(ByteBuffer source, CharBuffer target, IntBuffer offsets, boolean flush, CharsetDecoderListener cdl) {
            CoderResult[] cr = { CoderResult.UNDERFLOW };
            ...
            if (preToULength > 0) {
                 ...
                 * continuous offsets
                 */
                cr[0] = continueMatchToU(source, target, offsets, -1, flush, cdl);
            ...
            if (sharedData.mbcs.countStates == 1) {
                if ((sharedData.mbcs.unicodeMask & UConverterConstants.HAS_SUPPLEMENTARY) == 0) {
                    cr[0] = cnvMBCSSingleToBMPWithOffsets(source, target, offsets, flush, cdl);
                } else {
                    cr[0] = cnvMBCSSingleToUnicodeWithOffsets(source, target, offsets, flush, cdl);
                }
                return cr[0];
            } 
            ...
            while (sourceArrayIndex < source.limit()) {
                ...
                if (byteIndex == 0) {
                    ...
                    do {
                    	byte byte_b = source.get(sourceArrayIndex);
                        entry = stateTable[state][byte_b&UConverterConstants.UNSIGNED_BYTE_MASK];
                        if (cdl!=null)
                        	cdl.push(byte_b);
                        if (MBCS_ENTRY_IS_TRANSITION(entry)) {
                            ...
                            if (sourceArrayIndex < source.limit()) {
	                            byte_b = source.get(sourceArrayIndex);
	                            if (cdl!=null)
	                            	cdl.push(byte_b);
                            }
                            if (sourceArrayIndex < source.limit()
                                    && MBCS_ENTRY_IS_FINAL(entry = stateTable[state][byte_b&UConverterConstants.UNSIGNED_BYTE_MASK])
                            ...
                            if (MBCS_ENTRY_FINAL_IS_VALID_DIRECT_16(entry)) {
                                ...
                                if (cdl!=null)
                                	cdl.decoded(MBCS_ENTRY_FINAL_VALUE_16(entry));
                                if (offsets != null) {
                                    offsets.put(sourceIndex);
                    ...
                    if (cdl!=null)
                    	cdl.push(byte_b);
                    entry = stateTable[state][(bytes[byteIndex++] = byte_b)
                ...
                if (action == MBCS_STATE_VALID_16) {
                    ...
                    if (c < 0xd800) {
                        ...
                        if (target.hasRemaining()) {
                            ...
                            if (cdl!=null)
                            	cdl.decoded(unicodeCodeUnits[offset]);
                            if (offsets != null) {
                                offsets.put(sourceIndex);
                            }
                        } else {
                    ...
                    if (cdl!=null)
                    	cdl.decoded((char)(0xd800 | (char)(entry >> 10)));
                    if (offsets != null) {
                        offsets.put(sourceIndex);
                    }
                    byteIndex = 0;
                ...
                if (byteIndex == 0) {
                    ...
                    byteIndex = toU(byteIndex, source, target, offsets, sourceIndex, flush, cr, cdl);
        ...
        private CoderResult cnvMBCSSingleToBMPWithOffsets(ByteBuffer source, CharBuffer target, IntBuffer offsets,
            ...
            while (targetCapacity > 0 && sourceArrayIndex < source.limit()) {
            	...
            	if (cdl!=null)
            		cdl.push(byte_b);
                entry = stateTable[0][byte_b & UConverterConstants.UNSIGNED_BYTE_MASK];
                ...
                if (cr[0].isError()) {
                    ...
                    /* try an extension mapping */
                    lastSource = sourceArrayIndex;
                    byte_b = source.get(sourceArrayIndex - 1);
                    toUBytesArray[0] = byte_b;
                    if (cdl!=null)
                    	cdl.push(byte_b);
                    source.position(sourceArrayIndex);
                    toULength = toU((byte) 1, source, target, offsets, sourceIndex, flush, cr, cdl);
                    sourceArrayIndex = source.position();
         ...
         */
        int simpleGetNextUChar(ByteBuffer source, boolean useFallback, CharsetDecoderListener cdl) {
            ...
            if (c == 0xfffe) {
                ...
                if (sharedData.mbcs.extIndexes != null) {
                    ...
                    return simpleMatchToU(source, useFallback, cdl);
                }
            }
