...
public abstract class CharsetDecoderICU extends CharsetDecoder{ 
    ...
    protected final CoderResult implFlush(CharBuffer out) {
        return decode(EMPTY, out, null, true, null);
    }
    ...
    public interface CharsetDecoderListener {
    	static final int NO_CONFIDENCE_SET = -1;
    	void decoded(byte[] in, char c);
    	void decoded(byte in, char c);
    	void decoded(char c);
    	void push(byte b);
    	void push(byte[] b);
    }
    protected static final int ST_RESET   = 0;
    protected static final int ST_CODING  = 1;
    protected static final int ST_END     = 2;
    protected static final int ST_FLUSHED = 3;
    protected static String stateNames[]
    		= { "RESET", "CODING", "CODING_END", "FLUSHED" };
    private int state = ST_RESET;
    public final CoderResult decode(ByteBuffer in, CharBuffer out,
		    boolean endOfInput, CharsetDecoderListener cdl)
{
int newState = endOfInput ? ST_END : ST_CODING;
if ((state != ST_RESET) && (state != ST_CODING)
&& !(endOfInput && (state == ST_END)))
	throw new IllegalStateException("Current state = " + stateNames[state]
			+ ", new state = " + stateNames[newState]);
state = newState;
...
assert false;
    ...
    protected CoderResult decodeLoop(ByteBuffer in,CharBuffer out){
    	return decodeLoop(in, out, null);
    }
    
    protected CoderResult decodeLoop(ByteBuffer in,CharBuffer out, CharsetDecoderListener cdl){
...
//        }
        ...
        /* do the conversion */
        CoderResult ret = decode(in, out, null, false, cdl);
     ...
     *         action succeeds or more input is needed for completing the decoding action.
     */
    abstract CoderResult decodeLoop(ByteBuffer in, CharBuffer out, IntBuffer offsets, boolean flush, CharsetDecoderListener cdl);
     ...
     *         action succeeds or more input is needed for completing the decoding action.
     */
    final CoderResult decode(ByteBuffer source, CharBuffer target, IntBuffer offsets, boolean flush, CharsetDecoderListener cdl) {
        ...
        return toUnicodeWithCallback(source, target, offsets, flush, cdl);
    }
    ...
    final CoderResult toUnicodeWithCallback(ByteBuffer source, CharBuffer target, IntBuffer offsets, boolean flush, CharsetDecoderListener cdl){
        ...
        for(;;) {
            ...
            /* convert */
            cr = decodeLoop(source, target, offsets, flush, cdl);
