        ...
        protected CoderResult decodeLoop(ByteBuffer source, CharBuffer target, IntBuffer offsets, boolean flush, CharsetDecoderListener cdl) { 
            ...
            while (source.hasRemaining()) {
                ...
                if (target.hasRemaining()) {
                	...
                	if (cdl!=null)
                		cdl.push(byte_b);
                    sourceChar = (short)((short)byte_b & UConverterConstants.UNSIGNED_BYTE_MASK);
                            ...
                            if (sourceChar == 0xBF || sourceChar == 0xB8) {
                                ...
                                if ((validityTable[((short)targetUniChar) & UConverterConstants.UNSIGNED_BYTE_MASK] & data.currentMaskToUnicode) > 0) {
                                    ...
                                    if (data.prevToUnicodeStatus != 0) {
                                        cr = WriteToTargetToU(offsets, (source.position() - 1), source, target, data.prevToUnicodeStatus, (short)0, cdl);
                                        data.prevToUnicodeStatus = 0x0000;
                                    }
                                    /* write to target */
                                    cr = WriteToTargetToU(offsets, (source.position() - 2), source, target, targetUniChar, data.currentDeltaToUnicode, cdl);
                    ...
                    if (!gotoCallBack) {
                        ...
                        case ATR:
                            ...
                            if (this.toUnicodeStatus != UConverterConstants.missingCharMarker) {
                                ...
                                cr = WriteToTargetToU(offsets, (source.position() - 2), source, target, this.toUnicodeStatus, data.currentDeltaToUnicode, cdl); 
                        ...
                        case ISCII_NUKTA:
                            ...
                            if (data.contextCharToUnicode == ISCII_HALANT) {
                                 ...
                                 */
                                cr = WriteToTargetToU(offsets, (source.position() - 2), source, target, PNJ_RRA, (short)0, cdl);
                                if (!cr.isOverflow()) {
                                    cr = WriteToTargetToU(offsets, (source.position() - 2), source, target, PNJ_SIGN_VIRAMA, (short)0, cdl);
                                    if (!cr.isOverflow()) {
                                        cr = WriteToTargetToU(offsets, (source.position() - 2), source, target, PNJ_HA, (short)0, cdl);
                                    } else {
                    ...
                    if (!gotoCallBack && this.toUnicodeStatus != UConverterConstants.missingCharMarker) {
                        ...
                        if (data.currentDeltaToUnicode == PNJ_DELTA && data.prevToUnicodeStatus != 0 && PNJ_CONSONANT_SET.contains(data.prevToUnicodeStatus) &&
                            ...
                            cr = WriteToTargetToU(offsets, offset, source, target, PNJ_ADHAK, (short)0, cdl);
                            cr = WriteToTargetToU(offsets, offset, source, target, data.prevToUnicodeStatus, (short)0, cdl);
                            ...
                            if (data.currentDeltaToUnicode == PNJ_DELTA  && (targetUniChar + PNJ_DELTA) == PNJ_BINDI && PNJ_BINDI_TIPPI_SET.contains(this.toUnicodeStatus + PNJ_DELTA)) {
                                ...
                                cr = WriteToTargetToU(offsets, (source.position() - 2), source, target, this.toUnicodeStatus, PNJ_DELTA, cdl);
...
//CallBack :
            ...
            if (cr.isUnderflow() && flush && !source.hasRemaining()) {
                ...
                if (this.toUnicodeStatus != UConverterConstants.missingCharMarker) {
                    ...
                    WriteToTargetToU(offsets, (source.position() - 2), source, target, this.toUnicodeStatus, data.currentDeltaToUnicode, cdl);
                    this.toUnicodeStatus = UConverterConstants.missingCharMarker;    
                }
            }
            return cr;
        ...
        private CoderResult WriteToTargetToU(IntBuffer offsets, int offset, ByteBuffer source, CharBuffer target, int targetUniChar, short delta, CharsetDecoderListener cdl) {
            CoderResult cr = CoderResult.UNDERFLOW;
