...
public class RETest
{
    ...
    private void testSubst()
    {
        ...
        r = new RE("fo(o)");
        actual = r.subst("foo",
                         "$1", RE.REPLACE_BACKREFERENCES);
        assertEquals("Wrong subst() result", "o", actual);

        // Test for Bug #36405
        r = new RE("^(.*?)(x)?$");
        actual = r.subst("abc",
                         "$1-$2", RE.REPLACE_BACKREFERENCES);
        assertEquals("Wrong subst() result", "abc-", actual);

        r = new RE("^(.*?)(x)?$");
        actual = r.subst("abcx",
                         "$1-$2", RE.REPLACE_BACKREFERENCES);
        assertEquals("Wrong subst() result", "abc-x", actual);

        r = new RE("([a-b]+?)([c-d]+)");
        actual = r.subst("zzabcdzz",
                         "$1-$2", RE.REPLACE_BACKREFERENCES);
        assertEquals("Wrong subst() result", "zzab-cdzz", actual);
    }
...
final class RETestCase
{
    ...
    private void testMatch()
    {
        ...
        try
        {
            ...
            log.append("   Matched: ").append(result ? "YES" : "NO").append("\n");
            ...
            if (checkResult(result) && (!shouldMatch || checkParens()))
            {
                ...
                if (!tryMatchUsingCI(new StreamCharacterIterator(new ByteArrayInputStream(toMatch.getBytes()))))
                    return;
    ...
    private boolean checkParens()
    {
        ...
        log.append("   Paren count: ").append(regexp.getParenCount()).append("\n");
        ...
        {
            log.append("   Paren ").append(p).append(": ").append(regexp.getParen(p)).append("\n");
    ...
    boolean tryMatchUsingCI(CharacterIterator matchAgainst)
    {
        try {
            ...
            log.append("   Match: ").append(result ? "YES" : "NO").append("\n");
