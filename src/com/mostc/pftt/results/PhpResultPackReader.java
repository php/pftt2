...
public class PhpResultPackReader extends PhpResultPack {
	 ...
	 */
	public static PhpResultPackReader open(AHost host, File last_file) throws FileNotFoundException {
		return new PhpResultPackReader(host, last_file, PhptTallyFile.open(new File(last_file, "tally.xml")));
	}
	//
	protected PhptTallyFile tally;
	protected HashMap<EPhptTestStatus,BufferedReader> readers;

	protected PhpResultPackReader(AHost host, File telem_dir, PhptTallyFile tally) throws FileNotFoundException {
		super(host);
		this.tally = tally;
