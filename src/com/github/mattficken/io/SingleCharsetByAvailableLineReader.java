...
public class SingleCharsetByAvailableLineReader extends AbstractSingleCharsetAvailableReader implements ByLineReader {
	protected final byte[] bbuf;
	protected byte[] line_buf;
	protected int bbuf_off, bbuf_len, line_buf_len;
	
	public SingleCharsetByAvailableLineReader(InputStream in, CharsetDeciderDecoder cdd) {
		this(in, cdd, DEFAULT_READ_SIZE, DEFAULT_LINE_SIZE);
	}
	
	public SingleCharsetByAvailableLineReader(InputStream in, CharsetDeciderDecoder cdd, int read_size, int line_size) {
		super(in, cdd);
