...
public class SMTPResponseParser {
        ...
        private static final Pattern responsePattern =
            Pattern.compile("(\\d+)" + // group 1 = response code
        "(-| )?" + // group 2 = hyphen
    "([^\r\n]+)?\r\n"); // group 3 = response message

        private static final Pattern greetingPattern =
            Pattern.compile("^([^\\s]*)" + // group 1 = domain
    " ?(.*)$"); // group 2 = message
    ...
    public static SMTPResponse parse(CharSequence in) throws ParserException {
        ...
        if (matcher.matches()) {
            ...
            if (code != 220) {
                ...
                // find domain
                Matcher greetingMatcher =
                    greetingPattern.matcher(matcher.group(3));
